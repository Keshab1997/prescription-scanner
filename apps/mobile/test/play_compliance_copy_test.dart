...
void main() {
  test(
    'privacy copy discloses off-device AI and does not claim the image never leaves',
    () {
      ...
      expect(
        LegalCopy.privacyPolicy.toLowerCase(),
  ...
  test('medical copy forbids diagnosis and treatment', () {
    ...
    expect(
      LegalCopy.medicalFull.toLowerCase(),
  ...
  test('camera rationale explains prescription photos only', () {
    ...
    expect(
      LegalCopy.cameraRationale.toLowerCase(),
      contains('not saved to our cloud'),
    );
  });
